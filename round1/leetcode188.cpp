    ...
    //local[i][j], maximal profit with j transactions, the last one happends at i
    //global[i][j], maximal profit with j transaction, till i (not necessarily ending at i)
...
class Solution {
public:
    //if the number of transactions allowed (k) is larger than or equal the number of days
    ...
    int maxProfit(int k, vector<int>& prices) {
        ...
        local.resize(prices.size(), vector<int>(k+1, 0));
        //resize(num_of_row, vector<int> (num_of_col, init))
        vector<vector<int>> global;
        global.resize(prices.size(), vector<int>(k+1, 0));
        
        //another method to use 2-D array
        //int a[row][col] or
