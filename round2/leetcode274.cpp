...
public:
    ...
    int hIndex_time(vector<int>& citations) {
        ...
        int i = 0;
        for (int i = 0; i < n; i ++) {
            if (citations[i] < i+1)//here, i is the (i+1) th paper, the (i+1)th paper's citation is small than (i+1), so the H index is (i)
                return i;
        }
        return n;
    }
    int hIndex(vector<int>& citations) {
        ...
        int n = citations.size();
        for (int i = 0; i < n; i ++) {
            if (citations[i] >= i+1)//i+1 paper has citations >= i + 1
                continue;
            else
                return i;
        }
        //if never return in the for loop
        return n;
    }
    //O(n) time and O(n) space solution: ref; https://leetcode.com/discuss/73577/simple-java-o-n-time-o-n-space-solution
    int hIndex__hashtable(vector<int>& citations) {
        if (citations.empty())
            return 0;
