
class Solution {
public:
    //ref:https://leetcode.com/discuss/24013/my-short-c-solution-o-1-space-and-o-n-time
    //it seems that counting sort also works here, althought it is not constant space
    /*
    无序数组的题目如果要O(n)解法往往要用到hash table，但这题要求constant space。所以可以用数组本身作为一个"hash table"：A[0] = 1, A[1] = 2, .... A[n-1] = n。目标是尽可能将数字i放到数组第i-1个位置。

扫描数组中每个数：
1. 如果A[i]<1或者A[i]>n。说明A[i]一定不是first missing positive。跳过
2. 如果A[i] = i+1，说明A[i]已经在正确的位置，跳过
3. 如果A[i]!=i+1，且0<A[i]<=n，应当将A[i]放到A[A[i]-1]的位置，所以可以交换两数。
这里注意，当A[i] = A[A[i]-1]时会陷入死循环。这种情况下直接跳过。
    */
    int firstMissingPositive(vector<int>& nums) {
        for (int i = 0; i < nums.size(); i ++) {
            while (nums[i] != i + 1 && nums[i] > 0 && nums[i] <= nums.size() && nums[i] != nums[nums[i]-1])
                swap(nums[i], nums[nums[i]-1]);
        }
        for (int i = 0; i < nums.size(); i ++) {
            cout << nums[i] << " ";
        }
        cout << endl;
        int i;
        for (i = 0; i < nums.size(); i ++)
            if (nums[i] != i+1)
                return (i + 1);
        return nums.size()+1;
    }
    
    //can use unordered_set to solve this problem, but this is not O(1) space, timt complexity is O(n)
    int firstMissingPositive_usingset(vector<int>& nums) {
        unordered_set<int> se;
        for (auto x : nums)
            se.insert(x);
        for (int i = 1; i <= nums.size(); i ++)
            if (se.find(i) == se.end())
                return i;
        return nums.size()+1;
    }
};
